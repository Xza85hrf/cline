...
import { DeepClineProvider, GlobalFileNames } from "./webview/ClineProvider"
...
export class Cline {
	...
	private providerRef: WeakRef<DeepClineProvider>
	...
	async *attemptApiRequest(previousApiReqIndex: number): ApiStream {
		...
		if (previousApiReqIndex >= 0) {
			...
			if (previousRequest && previousRequest.text) {
				...
				const contextWindow = this.api.getModel().info.contextLength || 128_000
	...
	async presentAssistantMessage() {
		...
		switch (block.type) {
							...
							if (block.partial) {
								...
								const toolResultPretty =
									(toolResult?.isError ? "Error:\n" : "") +
										toolResult?.content
           .map((item: { type: string; text?: string; resource?: { blob?: string; [key: string]: any } }) => {
												if (item.type === "text" && item.text) {
													return item.text
												}
												if (item.type === "resource" && item.resource) {
													const { blob, ...rest } = item.resource as { blob?: string; [key: string]: any }
          ...
          .map((item: { text?: string; blob?: string; [key: string]: any }) => {
											if (item.text) {
												return item.text
											}
						...
						try {
							...
							break
								...
								case "testPrompt": {
									...
									try {
										if (block.partial) {
											await this.ask("test_prompt", removeClosingTag("prompt", prompt), block.partial).catch(() => {});
											break;
										} else {
											if (!prompt) {
												this.consecutiveMistakeCount++;
												pushToolResult(await this.sayAndCreateMissingParamError("testPrompt", "prompt"));
												break;
											}
											this.consecutiveMistakeCount = 0;
											...
											if (this.shouldAutoApproveTool(block.name)) {
												this.removeLastPartialMessageIfExistsWithType("ask", "test_prompt");
												await this.say("test_prompt", completeMessage, undefined, false);
												this.consecutiveAutoApprovedRequestsCount++;
											} else {
												showNotificationForApprovalIfAutoApprovalEnabled(
													`Cline wants to test a prompt: ${prompt}`
												);
												this.removeLastPartialMessageIfExistsWithType("say", "test_prompt");
												const didApprove = await askApproval("test_prompt", completeMessage);
												if (!didApprove) {
													break;
												}
											}
											...
											const testResult = await this.api.createMessage(prompt, []);
											let resultText = "";
											for await (const chunk of testResult) {
												if (chunk.type === "text") {
											...
											pushToolResult(formatResponse.toolResult(
												`Test prompt executed successfully. Here are the results:\n\n` +
												`<test_prompt>\n${prompt}\n</test_prompt>\n\n` +
												`<test_result>\n${resultText}\n</test_result>`
											));
											break;
							...
							await handleError("testing prompt", error)
							break
						}
					}
					case "attempt_completion": {
	...
	async recursivelyMakeClineRequests(
		...
		if (this.consecutiveMistakeCount >= 3) {
			...
			const { response, text, images } = await this.ask(
				...
				`This may indicate a failure in the thought process or inability to use a tool properly. You can help by providing guidance (e.g. "Try breaking down the task into smaller steps") or switching to a more capable model if needed.`,
			)
			if (response === "messageResponse") {
				userContent.push(
					...[
